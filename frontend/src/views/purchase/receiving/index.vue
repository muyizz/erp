...
const { loading, data, total, page, pageSize, searchParams, loadData, handlePageChange, handleSizeChange, handleSearch, handleReset } =
  useTable((p: any) => request.get('/purchase/receivings', { params: p }), { docNo: '', supplierId: undefined, orderNo: '', dateRange: [] as string[] })
...
async function loadOrderItemsForReceive(orderId: number) {
  try {
...
async function handleOrderChange(orderId: number) {
  if (orderId) {
    await loadOrderItemsForReceive(orderId)
  }
}
...
async function handleSubmit() {
  ...
  try {
    await request.post('/purchase/receivings', { header: { ...formData }, items: items.value })
...
function handleConfirm(row: any) {
  ElMessageBox.confirm('确认收货后库存将增加，确定继续？', '提示', { type: 'warning' }).then(async () => {
    await request.post(`/purchase/receivings/${row.id}/confirm`)
...
<template>
  <div>
    <div class="search-bar">
      <el-input v-model="searchParams.docNo" placeholder="收货单号" clearable style="width: 160px" />
      <el-input v-model="searchParams.orderNo" placeholder="采购订单号" clearable style="width: 160px" />
    ...
    <el-table :data="data" v-loading="loading" border stripe>
      ...
      <el-table-column prop="docNo" label="收货单号" width="150" />
      ...
      <el-table-column prop="receivedAt" label="收货日期" width="170" />
      <el-table-column label="操作" width="160" fixed="right">
        <template #default="{ row }">
          <el-button v-if="row.status === 1" size="small" type="success" @click="handleConfirm(row)">确认收货</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- Create Dialog -->
    <el-dialog title="新增收货单" v-model="visible" width="800px">
      <el-form ref="formRef" :model="formData" label-width="80px">
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="收货仓库" prop="warehouseId" :rules="[{ required: true, message: '请选择仓库' }]">
        ...
        <el-form-item label="收货日期" prop="receiveDate">
          <el-date-picker v-model="formData.receiveDate" type="date" value-format="YYYY-MM-DD" placeholder="选择日期" style="width: 100%" />
        </el-form-item>
      ...
      <el-table :data="items" border size="small">
        ...
        <el-table-column label="收货数量" width="120">
          <template #default="{ $index }">
            <el-input-number v-model="items[$index].receiveQty" :min="0" :max="items[$index].orderQty" size="small" style="width: 100%" />
          </template>
        </el-table-column>
      </el-table>
