...
const { loading, data, total, page, pageSize, searchParams, loadData, handlePageChange, handleSizeChange, handleSearch, handleReset } =
  useTable((p: any) => request.get('/inventory/stock-ins', { params: p }), { warehouseId: null, docType: null })

const docTypeMap: Record<number, string> = { 1: '采购入库', 2: '销售退货', 3: '盘盈入库', 4: '生产入库', 5: '调拨入库', 6: '期初入库' }

function handleConfirm(row: any) {
  ElMessageBox.confirm('确认后库存将增加，确定继续？', '提示', { type: 'warning' }).then(async () => {
    await request.post(`/inventory/stock-ins/${row.id}/confirm`)
