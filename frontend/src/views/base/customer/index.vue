...
const { loading, data, total, page, pageSize, searchParams, loadData, handlePageChange, handleSizeChange, handleSearch, handleReset } =
  useTable((p: any) => request.get('/base/customers', { params: p }), { customerCode: '', customerName: '', status: null })

const { visible, loading: formLoading, isEdit, formRef, formData, open, close, submit } =
  useForm(async (d) => {
    const url = d.isEdit ? `/base/customers/${d.id}` : '/base/customers'
    await request({ method: d.isEdit ? 'put' : 'post', url, data: d })
    ElMessage.success(d.isEdit ? '修改成功' : '新增成功')
    loadData()
  }, { customerCode: '', name: '', contact: '', phone: '', email: '', address: '', remark: '', status: 1 })

function handleDelete(row: any) {
  ElMessageBox.confirm('确定删除该客户吗？', '提示', { type: 'warning' }).then(async () => {
    await request.delete(`/base/customers/${row.id}`)
...
<template>
  <div>
    <div class="search-bar">
      <el-input v-model="searchParams.customerCode" placeholder="客户编码" clearable style="width: 160px" />
      <el-input v-model="searchParams.customerName" placeholder="客户名称" clearable style="width: 160px" />
    ...
    <div class="toolbar">
      <el-button type="primary" @click="open(false)">新增客户</el-button>
    </div>
    ...
    <el-dialog :title="isEdit ? '编辑客户' : '新增客户'" v-model="visible" width="580px" @close="close">
      <el-form ref="formRef" :model="formData" label-width="90px">
        <el-form-item label="客户编码" prop="customerCode" :rules="[{ required: true, message: '请输入编码' }]">
          <el-input v-model="formData.customerCode" :disabled="isEdit" />
        </el-form-item>
        <el-form-item label="客户名称" prop="customerName" :rules="[{ required: true, message: '请输入名称' }]">
