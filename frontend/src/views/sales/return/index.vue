...
const { loading, data, total, page, pageSize, searchParams, loadData, handlePageChange, handleSizeChange, handleSearch, handleReset } =
  useTable((p: any) => request.get('/sales/returns', { params: p }), { docNo: '', customerId: undefined, dateRange: [] as string[] })
...
async function handleSubmit() {
  ...
  try {
    await request.post('/sales/returns', { header: { ...formData }, items: items.value })
...
function handleConfirm(row: any) {
  ElMessageBox.confirm('确认退货后库存将增加，确定继续？', '提示', { type: 'warning' }).then(async () => {
    await request.post(`/sales/returns/${row.id}/confirm`)
...
<template>
  <div>
    ...
    <el-table :data="data" v-loading="loading" border stripe>
      ...
      <el-table-column prop="deliveryNo" label="关联发货单" width="150" />
    ...
    <el-dialog title="新增退货单" v-model="visible" width="800px">
      <el-form ref="formRef" :model="formData" label-width="100px">
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="关联发货单" prop="deliveryId" :rules="[{ required: true, message: '请输入发货单ID' }]">
              <el-input-number v-model="formData.deliveryId" :min="1" placeholder="发货单ID" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
