...
const { loading, data, total, page, pageSize, searchParams, loadData, handlePageChange, handleSizeChange, handleSearch, handleReset } =
  useTable((p: any) => request.get('/sales/orders', { params: p }), { orderNo: '', customerId: undefined, status: null, dateRange: [] as string[] })
...
async function handleSubmit() {
  ...
  try {
    const url = isEdit.value ? `/sales/orders/${formData.id}` : '/sales/orders'
...
function handleApprove(row: any) {
  ElMessageBox.confirm('确定审批通过该销售订单吗？', '提示', { type: 'warning' }).then(async () => {
    await request.post(`/sales/orders/${row.id}/approve`)
...
function handleCancel(row: any) {
  ElMessageBox.confirm('确定取消该销售订单吗？', '提示', { type: 'warning' }).then(async () => {
...
function handleDelete(row: any) {
  ElMessageBox.confirm('确定删除该销售订单吗？', '提示', { type: 'warning' }).then(async () => {
...
<template>
  <div>
    ...
    <el-dialog :title="isEdit ? '编辑销售订单' : '新增销售订单'" v-model="visible" width="800px" @close="visible = false">
      <el-form ref="formRef" :model="formData" label-width="80px">
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="客户" prop="customerId" :rules="[{ required: true, message: '请选择客户' }]">
              <el-input-number v-model="formData.customerId" :min="1" placeholder="客户ID" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
