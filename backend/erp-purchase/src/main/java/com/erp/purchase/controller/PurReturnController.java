...
public class PurReturnController {
    ...
    private final PurReturnItemMapper itemMapper;
    private final InvStockMapper stockMapper;
    @GetMapping
    public R<PageResult<PurReturn>> list(@RequestParam(defaultValue = "1") long page,
            @RequestParam(defaultValue = "20") long pageSize, @RequestParam(required = false) Long supplierId,
            @RequestParam(required = false) Integer status) {
        var w = new LambdaQueryWrapper<PurReturn>()
                .eq(supplierId != null, PurReturn::getSupplierId, supplierId)
                .eq(status != null, PurReturn::getStatus, status).orderByDesc(PurReturn::getCreatedAt);
        Page<PurReturn> p = mapper.selectPage(new Page<>(page, pageSize), w);
    ...
    @GetMapping("/{id}") public R<Map<String,Object>> detail(@PathVariable Long id) {
        ...
        r.put("items", itemMapper.selectList(new LambdaQueryWrapper<PurReturnItem>().eq(PurReturnItem::getReturnId, id)));
        return R.ok(r);
    }
    @PostMapping public R<Void> create(@RequestBody Map<String,Object> body) {
        PurReturn h = parse(body.get("header"), PurReturn.class);
        h.setReturnNo("PRT-" + java.time.LocalDate.now().toString().replace("-","") + "-" + System.currentTimeMillis()%10000);
        h.setStatus(1); mapper.insert(h);
        saveItems(body, h.getId()); return R.ok();
    }
    @PostMapping("/{id}/confirm") @Transactional
    public R<Void> confirm(@PathVariable Long id) {
        PurReturn h = mapper.selectById(id);
        List<PurReturnItem> items = itemMapper.selectList(new LambdaQueryWrapper<PurReturnItem>().eq(PurReturnItem::getReturnId, id));
        for (PurReturnItem item : items) {
            stockMapper.deduct(item.getMaterialId(), 1L, item.getQuantity());
        }
    ...
    private void saveItems(Map<String,Object> body, Long rid) {
        ...
        for (Map<String,Object> im : items) {
            PurReturnItem item = parse(im, PurReturnItem.class);
            item.setReturnId(rid);
